...
all_selns <- Compile_Raven_bio_selns(site_id = site_id, 
                                 dep_id = dep_id)

# Save compiled selection table in outputs folder
write.table(all_selns, 
            paste0("output/", site_id,"_",dep_id,"_all_biology_selections.txt"),
            row.names = FALSE)
...
all_selns_hr <- all_selns |>
  mutate(Begin_Clock = as_datetime(Begin_Clock, format = "%H:%M:%OS"),
         ...
         ## Fix text errors/typos from selection tables
         # remove spaces
         ...
         Call_type = case_when(Bio_Category == "pygmyblue" | Bio_Category == "pygmybluewhale" ~ "pbw",
                               TRUE ~ Call_type),
         Bio_Category = case_when(Call_type == "pbw" ~ "bal", TRUE ~ Bio_Category),
...
hr_tally <- all_selns_hr |>
  ...
  #make new column for each combo of category and call type
  mutate(
    "category_calltype" = paste0(Bio_Category,"_",Call_type)) |>
  ...
  count() |>
  # pivot wider to get call types as columns and fill missing values with 0
...
all_bio_hr <- left_join(hr_tally, hr_dol_tally, by = c("Begin_Date", "Begin_Hour"))
...
write.csv(hourly_pres_notes, 
          paste0("output/",site_id,"_",dep_id,"_Bio_Hourly_Presence.csv"))
