...
psd_og <- read_csv("data/Df_analysis/All_deps_monthly_diel_PSD.csv")



# pivot wider to create new df with one column per deployment-category combination
psd_wide <- psd_og |>
    # change to simpler deployment names
  mutate(Dep_ID = str_replace(Dep_ID, "PARKSAUSTRALIA_", ""),
         month_totdays = days_in_month(month_UTC)) |>
  
  # only keep data from months with >= half the month
  # only keep freqs > 20Hz (flat freq response of STs)
  filter(effort_days >= 0.5*month_totdays,
         frequency >= 20)|>

  # drop quantiles not using
  select(Dep_ID, month_UTC, light_reg, frequency, qmed)|>
  # new col names = Deployment_month_diel
  pivot_wider(names_from = c(Dep_ID, month_UTC, light_reg), values_from = c(qmed))
...
psd_wide |>
  summarize(dissim = dissim_calc(spec1 = CGMP_201807_CG_7_day, 
              spec2 = SWC_202201_SWS_2_day, 
              freqs = frequency))
...
# woohoo!
# 
# ggsave("figs/dissim_matrix_dep_monthdiel_all.jpg", device = "jpeg",
...
write_csv(dissim_matrix,
          "output/dissim_matrix_dep_month_diel_20Hz.csv")
