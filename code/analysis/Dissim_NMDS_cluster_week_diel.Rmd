...
dissim_og <- read_csv("output/Weekly_dissim/dissim_matrix_dep_week_diel.csv")

# load HMD weekly data -- has deployment ID and ISO week info
all_dep_psd <- read_csv("data/Df_analysis/All_deps_weekly_diel_PSD.csv")

# get grouping info from original PSD data
all_dep_labels <- all_dep_psd |>
  select(network, npz_id, Dep_ID, isoweek_UTC,light_reg) |>
  ...
  mutate(dep = paste(Dep_ID, isoweek_UTC, light_reg, sep = "_"))
...
sdf_agnes_cut <- cutree(sdf_agnes, k = 10)
