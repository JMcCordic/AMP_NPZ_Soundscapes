...
# get hourly quantiles summarized over deployments for plotting
...
# get daily quantiles for peak detection
TOL10khz_day <- all_tol_data |>
  # remove rows with NA values
  filter(is.na(TOL_13) == FALSE) |>
  # group by Dep_ID and date, retain other grouping vars
  group_by(network, npz_id, Dep_ID, Latitude, Longitude, Begin_date_UTC) |>
  # calculate q10, q50, q90
  summarise(
    q10_tol = quantile(TOL_10000, 0.1),
    med_tol = quantile(TOL_10000, 0.5),
    q75_tol = quantile(TOL_10000, 0.75),
    q90_tol = quantile(TOL_10000, 0.9)
  )
...
TOL10khz_shrimp <- all_tol_data |>
  # remove rows with NA values
  filter(is.na(TOL_13) == FALSE) |>
  # group by Dep_ID and date, retain other grouping vars
  group_by(network, npz_id, Dep_ID, Latitude, Longitude, Begin_date_UTC) |>
  mutate(q90_tol = quantile(TOL_10000, 0.9),
         shr_pres = case_when(
           # South-west Corner had no daily presence of snapping shrimp from manual LTSA review
           npz_id == "South-west Corner" ~ 0, 
           # For remaining NPZs, if TOL > daily 90th percentile, label as shrimp present
           TOL_10000 >= q90_tol ~ 1,
           TOL_10000 < q90_tol ~ 0
         )) 

tol_shrimp_pres <- TOL10khz_shrimp |>
  select(Dep_ID, UTC, Latitude, Longitude, shr_pres) |>
  mutate(Dep_ID = str_replace(Dep_ID, "PARKSAUSTRALIA_", ""))

# export shrimp presence CSV
write_csv(tol_shrimp_pres, "output/TOL10khz_shrimp.csv")

# full time series of TOL values?
# this looks horrid...try single NPZ 
ggplot(data = TOL10khz_shrimp |> 
         filter(npz_id == "Ningaloo"), 
       aes(x = UTC,
           y = TOL_10000)) +
  geom_line()+
  geom_point(aes(color = factor(shr_pres)))+
  ...
  geom_hline(aes(yintercept = q90_tol,
                 color = Begin_date_UTC))
# 
