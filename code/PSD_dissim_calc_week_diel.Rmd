...
psd_og <- read_csv("data/Df_analysis/All_deps_weekly_diel_PSD.csv")
...
psd_wide <- psd_og |>
  ...
  select(Dep_ID, isoweek_UTC, light_reg, frequency, qmed)|>
  # new col names = Deployment_month_effortdays
  pivot_wider(names_from = c(Dep_ID, isoweek_UTC, light_reg), values_from = c(qmed))
...
write_csv(dissim_matrix,
          "output/dissim_matrix_dep_week_diel.csv")
